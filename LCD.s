...
wc_loop1
	LDR  R3, [R1]	                ; 1) 
	ANDS R3, #0x10                  ; 2) Use bitmask to isolate bit 4
	BNE  wc_loop1                   
    MOV  R3, #0                     ; 3) R3 holds 0 for DC
	STR  R3, [R2] 
	LDR  R3, =SSI0_DR_R             ; 4) Register mode
	STRB R0, [R3]                   
wc_loop2
    LDR  R3, [R1]                   ; 5)
	ANDS R3, #0x10                  ; 6) Use bitmask to isolate bit 4
	BNE  wc_loop2
...
wd_loop    
    LDR  R3, [R1]                    ; 1) 
    ANDS R3, #0x02                   ; 2) Use bitmask to isolate bit 1
    BEQ  wd_loop
    MOV  R3, #0x40                   ; 3) R3 (bit 6) is high for DC
    STR  R3, [R2]
    LDR  R3, =SSI0_DR_R              ; 4) 
    STRB R0, [R3] 
